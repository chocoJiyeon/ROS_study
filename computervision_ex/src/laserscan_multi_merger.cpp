...
void LaserscanMerger::laserscan_topic_parser()
{
	...
	while (!tokens.empty())
	{
		...
		{
			if (topics[i].datatype == "sensor_msgs/LaserScan" && tokens.erase(topics[i].name) > 0)	//laserscan 메시지를 다 받는듯?
			{
				tmp_input_topics.push_back(topics[i].name);
			}
		}
	}
	//sort -> unique -> erase 로 중복된 데이터 제거
	sort(tmp_input_topics.begin(), tmp_input_topics.end());
	std::vector<string>::iterator last = std::unique(tmp_input_topics.begin(), tmp_input_topics.end());
	tmp_input_topics.erase(last, tmp_input_topics.end());

	// Do not re-subscribe if the topics are the same     /laser scan 데이터 개수가 달라질 때만 업데이트
...
LaserscanMerger::LaserscanMerger()
{
	...
	nh.param<std::string>("destination_frame", destination_frame, "camera_01_link");
...
void LaserscanMerger::scanCallback(const sensor_msgs::LaserScan::ConstPtr &scan, std::string topic)
{
	...
	try
	{
		...
		projector_.transformLaserScanToPointCloud(scan->header.frame_id, *scan, tmpCloud1, tfListener_, laser_geometry::channel_option::Distance);	//laserscan -> pointcloud (tmpCloud1)
	...
	for (int i = 0; i < input_topics.size(); i++)
	{
		...
		{
			sensor_msgs::convertPointCloudToPointCloud2(tmpCloud2, tmpCloud3);	//tf된 pointcloud2 : tmpCloud3
			pcl_conversions::toPCL(tmpCloud3, clouds[i]);												//tf 변환한 pointcloud2 data : clouds (vector<pcl::pointcloud2>)
	...
	if (totalClouds == clouds_modified.size())
	{
		...
		getPointCloudAsEigen(merged_cloud, points);		//pointcloud2 data --> XYZ points
...
void LaserscanMerger::pointcloud_to_laserscan(Eigen::MatrixXf points, pcl::PCLPointCloud2 *merged_cloud)
{
	...
	for (int i = 0; i < points.cols(); i++)
	{
		...
		if (range_sq < range_min_sq_)		//depth 범위 확인
		{
		...
		if (angle < output->angle_min || angle > output->angle_max)	//angle 범위 확인 
		{
		...
		if (output->ranges[index] * output->ranges[index] > range_sq)		//가까운 거리값으로 업데이트 
